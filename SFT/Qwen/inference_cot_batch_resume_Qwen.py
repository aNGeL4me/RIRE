...
PROMPT_DICT = {
    "prompt_input": (
        ...
        "Input: {input}\n"
        "<|im_end|>\n"
        "<|im_start|>assistant\n"  # Qwen requires explicit assistant role
    ),
    "prompt_no_input": (
        "<|im_start|>user\n"
...
def parse_args():
    ...
    # If using a single GPU, set batch_size to 20 if using the full memory
    return parser.parse_args()

# Extracts [Vulnerable] or [Secure] from the model output and maps to 1/0
...
def extract_final_decision(response):
    ...
    # If "Vulnerable line: None" is present, return 0 (secure)
    if "Vulnerable line: None" in response:
        return 0
    # If "Vulnerable line: None" is not present, return 1 (vulnerable)
...
def fpr_fnr_score(y_true, y_pred):
    ...
    FPR = FP / (FP + TN) if (FP + TN) > 0 else 0.0  # Denominator is all samples with label 0: 0->1 and 0->0
    FNR = FN / (FN + TP) if (FN + TP) > 0 else 0.0  # Denominator is all samples with label 1: 1->0 and 1->1
    return FPR, FNR
...
def main():
    ...
    # Create CSV file or resume from existing one
