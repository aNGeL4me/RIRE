...
PROMPT_DICT = {
    "prompt_input": (
        ...
        "Input: {input}\n"
        "<|im_end|>\n"
        "<|im_start|>assistant\n"  # Qwen requires explicitly specifying the assistant role
    ),
    "prompt_no_input": (
        "<|im_start|>user\n"
...
def fpr_fnr_score(y_true, y_pred):
    ...
    # FPR denominator is all samples with label 0: 0->1 and 0->0 (label → prediction)
    # FNR denominator is all samples with label 1: 1->0 and 1->1 (label → prediction)
...
def main():
    ...
    # Get unprocessed index list
    all_indices = set(range(len(data)))
    unprocessed_indices = sorted(list(all_indices - processed_indices))
    current_pos = 0

    # Resume inference from interruption (e.g., OOM), can continue after adjusting batch size
    while current_pos < len(unprocessed_indices):  
        ...
        # Ensure prob_dists does not exceed bounds
        ...
        for response, confidence in zip(responses, confidence_scores):
            prediction = 0
            parsed_response = response.split("assistant")[-1].strip()
        ...
        # Ensure indices match data length
