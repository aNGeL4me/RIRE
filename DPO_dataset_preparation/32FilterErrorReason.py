...
def check_reason_field_endings(file_path, error_output_path):
    ...
    for elem in data:
        ...
        output = int(output) if output.isdigit() else -1  # Prevent errors from non-digit values
    ...
    print(f"Processing complete, found {len(errors)} format error samples, written to {error_output_path}")

# Example usage
check_reason_field_endings(
    file_path='train512_22837_randomSampled3970_0x01_3reasons_cleaned_updated_2nd.json',
    error_output_path='train512_22837_randomSampled3970_0x01_3reasons_cleaned_updated_error3.json'
)
