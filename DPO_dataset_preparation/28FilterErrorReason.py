...
def check_reason_field_endings(file_path, error_output_path):
    ...
    for elem in data:
        ...
        output = int(output) if output.isdigit() else -1  # Prevent error if not a digit
    ...
    print(f"Processing complete, found {len(errors)} format error samples, saved to {error_output_path}")

# Example usage
check_reason_field_endings(
    file_path='train512_22837_randomSampled3970_0x01_3reasons_cleaned.json',
    error_output_path='train512_22837_randomSampled3970_0x01_3reasons_error.json'
)
