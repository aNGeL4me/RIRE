...
def parse_args():
    ...
    parser.add_argument("--data_file", required=True, help="Path to the JSON file containing the test dataset.")
    ...
    # If using a single GPU fully, you can set batch_size to 20
    return parser.parse_args()


# Extract [Vulnerable] or [Secure] from CoT-generated responses and convert to 1/0
def extract_final_decision(response):
    ...
    # 2. If it contains "Vulnerable line: None", return 0
    if "Vulnerable line: None" in response:  # Analysis of the function
        return 0
    # 3. If it does not contain "Vulnerable line: None", return 1
    if "Vulnerable line: None" not in response:
        return 1
    # 4. Return None if unable to parse
    return None
...
def main():
    ...
    # Create CSV file: if not exists, create one; otherwise, read processed Index list
    ...
    current_pos = 0
    while current_pos < len(unprocessed_indices):  # Resume inference: when OOM happens, reduce batch size and continue from last point
    ...
    # Final evaluation: read all completed samples from full CSV file
    final_df = pd.read_csv(args.csv_path)
    all_labels = final_df['Label'].tolist()
    all_predictions = final_df['Prediction'].tolist()

    # Final evaluation metrics on all inference samples
