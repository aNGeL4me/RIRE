...
def clean_commit_message(message):
    signature_patterns = [
        ...
        r'Authorship / merged commits', r'Message-id', r'Review-by', r'Reviewed by', r'Suggested by', r'Signed-off by', r'Idea-by', r'Reported by'
    ]
...
def process_function_json(function_json_path, commits_base_dir):
    ...
    for element in elements:
        ...
        if not os.path.exists(commit_file):
            print(f"warning: commit file not exists {commit_file}")
            continue
        ...
        element['message'] = clean_commit_message(
    ...
    #     '202504190950_update_taraget0_' + os.path.basename(function_json_path)
    # )
    base_name = os.path.basename(function_json_path)
    name, ext = os.path.splitext(base_name)

    new_filename = f"{name}target0{ext}"
...
if __name__ == "__main__":
    # process_function_json('train_512_InDevign_AddNewKeysCommitidProject.json', '../../temp/commits_202503102000')
    process_function_json('train_512_InDevign_AddNewKeysCommitidProject_updated_target1.json', './commits_202503200950')
